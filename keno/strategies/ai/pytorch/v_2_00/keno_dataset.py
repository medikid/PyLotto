...
class KenoDataset(Dataset):
    ...
    def __init__(self, dataset_name, data_type):
        ...
        if (data_type=="train"):
            ...
            self.len = data['val'].shape[0]
            self.x_data = torch.from_numpy(data['val'])
        elif (data_type=="test"):
            self.len = data['test'].shape[0]
            self.x_data = torch.from_numpy(data['test'])
...
# for i in range(hsp.__len__()):
#     ids, inputs, targets = hsp.__getitem__(i);
#     print(ids)
